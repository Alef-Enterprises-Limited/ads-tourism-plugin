...
Custom CSS is administrator-only because it is part of the existing `manage_options` settings page. `CustomCssSanitizer` removes tags, NUL bytes, style/script breakouts, imports, JavaScript URLs, legacy expression/behavior constructs, and bounds storage to 50 KB. Larger designs belong in a child theme or versioned builder stylesheet.
